...
from model import single_step_schedule
from cyclegan_turbo import CycleGAN_Turbo, VAE_Encoder, VAE_Decoder, initialize_unet, initialize_vae
from my_utils.training_utils import UnpairedDataset, UnpairedFeatureDataset, build_transform, parse_args_unpaired_training
...
def main(args):
    ...
    noise_scheduler_1step = single_step_schedule()
    text_encoder = CLIPTextModel.from_pretrained("stabilityai/sd-turbo", subfolder="text_encoder").cuda()

    unet, l_modules_unet_encoder, l_modules_unet_decoder, l_modules_unet_others = initialize_unet(args.lora_rank_unet, return_lora_module_names=True, change_conv_in=(args.use_feature_conditioning and not args.use_wrapper))
    ...
    # Wrap the UNet with the conditional block
    ...
    vae_enc = VAE_Encoder(vae_a2b, vae_b2a=vae_b2a)
    vae_dec = VAE_Decoder(vae_a2b, vae_b2a=vae_b2a)
    ...
    if args.use_feature_conditioning:
        dataset_train = UnpairedFeatureDataset(dataset_folder=args.dataset_folder, image_prep=args.train_img_prep, split="train", tokenizer=tokenizer)
    else:
    ...
    # Perhaps wrapped UNet will error out here if accelerator is not properly handling the wrapped model?
    ...
    for epoch in range(first_epoch, args.max_train_epochs):
        for step, batch in enumerate(train_dataloader):
            ...
            with accelerator.accumulate(*l_acc):
                ...
                if args.use_feature_conditioning:
                    features_a = batch["features_src"].to(dtype=weight_dtype)
                    features_b = batch["features_tgt"].to(dtype=weight_dtype)
                else:
                ...
                """
                # A -> fake B -> rec A
                cyc_fake_b = CycleGAN_Turbo.forward_with_networks(img_a, "a2b", vae_enc, unet, vae_dec, noise_scheduler_1step, timesteps, fixed_a2b_emb, features=features_a, use_wrapper=args.use_wrapper)
                cyc_rec_a = CycleGAN_Turbo.forward_with_networks(cyc_fake_b, "b2a", vae_enc, unet, vae_dec, noise_scheduler_1step, timesteps, fixed_b2a_emb, features=features_a, use_wrapper=args.use_wrapper)
                ...
                # B -> fake A -> rec B
                cyc_fake_a = CycleGAN_Turbo.forward_with_networks(img_b, "b2a", vae_enc, unet, vae_dec, noise_scheduler_1step, timesteps, fixed_b2a_emb, features=features_b, use_wrapper=args.use_wrapper)
                cyc_rec_b = CycleGAN_Turbo.forward_with_networks(cyc_fake_a, "a2b", vae_enc, unet, vae_dec, noise_scheduler_1step, timesteps, fixed_a2b_emb, features=features_b, use_wrapper=args.use_wrapper)
                ...
                """
                fake_a = CycleGAN_Turbo.forward_with_networks(img_b, "b2a", vae_enc, unet, vae_dec, noise_scheduler_1step, timesteps, fixed_b2a_emb, features=features_b, use_wrapper=args.use_wrapper)
                fake_b = CycleGAN_Turbo.forward_with_networks(img_a, "a2b", vae_enc, unet, vae_dec, noise_scheduler_1step, timesteps, fixed_a2b_emb, features=features_a, use_wrapper=args.use_wrapper)
                ...
                Identity Objective
                """
                idt_a = CycleGAN_Turbo.forward_with_networks(img_b, "a2b", vae_enc, unet, vae_dec, noise_scheduler_1step, timesteps, fixed_a2b_emb, features=features_b, use_wrapper=args.use_wrapper)
                loss_idt_a = crit_idt(idt_a, img_b) * args.lambda_idt
                loss_idt_a += net_lpips(idt_a, img_b).mean() * args.lambda_idt_lpips
                idt_b = CycleGAN_Turbo.forward_with_networks(img_a, "b2a", vae_enc, unet, vae_dec, noise_scheduler_1step, timesteps, fixed_b2a_emb, features=features_a, use_wrapper=args.use_wrapper)
                        ...
                        for idx, input_img_path in enumerate(tqdm(l_images_src_test)):
                            ...
                            with torch.no_grad():
                                ...
                                eval_fake_b = CycleGAN_Turbo.forward_with_networks(img_a, "a2b", eval_vae_enc, eval_unet,
                                    eval_vae_dec, noise_scheduler_1step, _timesteps, fixed_a2b_emb[0:1], features=features_a, use_wrapper=args.use_wrapper)
                        ...
                        for idx, input_img_path in enumerate(tqdm(l_images_tgt_test)):
                            ...
                            with torch.no_grad():
                                ...
                                eval_fake_a = CycleGAN_Turbo.forward_with_networks(img_b, "b2a", eval_vae_enc, eval_unet,
                                    eval_vae_dec, noise_scheduler_1step, _timesteps, fixed_b2a_emb[0:1], features=features_b, use_wrapper=args.use_wrapper)
